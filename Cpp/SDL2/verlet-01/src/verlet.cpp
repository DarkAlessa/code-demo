...
SDL_Color setColor(float time) {
	...
	constexpr Uint8 rgb_min = 120;
	constexpr Uint8 limit = (rgb_max - rgb_min) / 2;
	constexpr Uint8 up = limit + rgb_min;
	Uint8 r = static_cast<Uint8>(up + limit * std::sin(time/limit - 0.5));
	Uint8 g = static_cast<Uint8>(up * limit * std::sin(time/limit + 0.5));
	Uint8 b = static_cast<Uint8>(up * limit * std::sin(time/limit + 1.0));
	return {r, g, b, 255};
}
...
struct Verlet {
	...
	SDL_FPoint vel   = {3.0f, 2.0f};
	SDL_FPoint acc   = {0.0f, 0.0f};
	SDL_FPoint force = {0.0f, 0.2f};
	float mass       = 1.0f;
		...
		while (SDL_PollEvent(&e)) {
	        switch (e.type) {
				...
				case SDL_EventType::SDL_MOUSEBUTTONDOWN:
					verlet.push_back(Verlet{
											SDL_FPoint{(float)(e.button.x - (32/2)), (float)(e.button.y - (32/2))},
